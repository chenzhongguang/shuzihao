  ...
  <div>
    <div class="time">
      ...
      <div class="time-rigth" @click="tendencyMapFn">
        ...
        <div :class="{ tendency: true, lista: listmap == 2 }" data-map="2">
          数据列表
        </div>
      </div>
    </div>
    ...
    <div :class="{ timecontent: true, timecontent1: timecontent1 }">
      <el-table :data="showTableData" class="label">
        ...
        <el-table-column prop="article" label="文章收益" />
        <el-table-column prop="video" label="视频收益" />
      </el-table>
      <div class="next">
  ...
  watch: {
    ...
    tableData() {
      (this.option.xAxis.data = this.savedarwdata.addtime), //total
        (this.option.series[0].data = this.savedarwdata.createarn), //创作收益
        (this.option.series[1].data = this.savedarwdata.article), //文章收益
        (this.option.series[2].data = this.savedarwdata.video), //视频收益
        // 自动获取的三十条数据把时条给当前图
    ...
    "day.day"() {
      ...
      this.option.series[0].data = this.savedarwdata.createarn.slice(
        0,
        this.day.day
      ); //total
      this.option.series[1].data = this.savedarwdata.article.slice(
        0,
        this.day.day
      ); //createarn
      this.option.series[2].data = this.savedarwdata.video.slice(
        0,
        this.day.day
      ); //other
      // 监听点击的天数
      ...
      creationdata(day)
        .then((res) => {
          ...
          res.data.forEach((item: objectI) => {
            ...
            //时间格式化
            item.addtime = (item.addtime as unknown as string).substring(
              0,
              10
            ) as never; //日期
            ...
            this.Object2.addtime = item.addtime;
            // 创作收益
            this.savedarwdata.createarn.unshift(((item.createarn as unknown as number) / 1) + ((item.video as unknown as number) / 1) as unknown as never); //总计
            this.Object2.createarn = ((item.createarn as unknown as number) / 1) + ((item.video as unknown as number) / 1) as unknown as string;
            // 文章收益
            this.savedarwdata.article.unshift(
              ((item.createarn as unknown as number) / 1) + ((item.basearn as unknown as number) / 1) + ((item.subsidy as unknown as number) / 1) + ((item.other as unknown as number) / 1) as never
            );
            this.Object2.article = ((item.createarn as unknown as number) / 1) + ((item.basearn as unknown as number) / 1) + ((item.subsidy as unknown as number) / 1) + ((item.other as unknown as number) / 1) as unknown as string;
            //视频收益
            this.savedarwdata.video.unshift(((item.video as unknown as number) / 1) + ((item.videobasearn as unknown as number) / 1) + ((item.videosubsidy as unknown as number) / 1) + ((item.othervideo as unknown as number) / 1) as never);
            this.Object2.video = ((item.video as unknown as number) / 1) + ((item.videobasearn as unknown as number) / 1) + ((item.videosubsidy as unknown as number) / 1) + ((item.othervideo as unknown as number) / 1) as unknown as string;
            this.tableData1.push(this.Object2 as never);
          });
